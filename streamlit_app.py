...
# --- MODULE MUR À CONTREFORTS ---
    ...
    with tab3:
        ...
        st.success(f"Béton Total : {vol_global:.2f} m³ | Acier Total : {poids_acier_global:.0f} kg")

    # --- CALCULATEUR MATÉRIAUX ---
    st.divider()
    st.header("📊 Calculateur de Matériaux")
    volume_final = st.number_input("Volume total de béton (m³)", value=float(vol_global))
    dosage = st.selectbox("Dosage Ciment (kg/m³)", [300, 350, 400], index=1)
    
    nb_sacs = (volume_final * dosage) / 50
    v_sable = volume_final * 0.4
    v_gravier = volume_final * 0.8
    
    m1, m2, m3 = st.columns(3)
    m1.metric("Ciment (Sacs)", f"{nb_sacs:.0f}")
    m2.metric("Sable (m³)", f"{v_sable:.2f}")
    m3.metric("Gravier (m³)", f"{v_gravier:.2f}")
